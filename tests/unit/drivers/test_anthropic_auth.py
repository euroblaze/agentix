"""Unit tests for agentix.drivers.adapters.intrinsic.anthropic_auth token sources."""
...
from agentix.drivers.adapters.intrinsic.anthropic_auth import (
    ChainTokenSource,
    EnvTokenSource,
    FileTokenSource,
    KeychainTokenSource,
...
def test_keychain_token_source_happy_path() -> None:
    ...
    with patch(
        "agentix.drivers.adapters.intrinsic.anthropic_auth.subprocess.run", return_value=_fake_completed(stdout=payload)
    ) as mock:
...
def test_keychain_token_source_security_missing() -> None:
    ...
    with patch("agentix.drivers.adapters.intrinsic.anthropic_auth.subprocess.run", side_effect=FileNotFoundError):
...
def test_keychain_token_source_user_denied() -> None:
    ...
    with patch(
        "agentix.drivers.adapters.intrinsic.anthropic_auth.subprocess.run",
...
def test_resolve_keychain_then_file(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with patch(
        "agentix.drivers.adapters.intrinsic.anthropic_auth.subprocess.run",
        return_value=_fake_completed(stdout=kc_payload),
    ):
